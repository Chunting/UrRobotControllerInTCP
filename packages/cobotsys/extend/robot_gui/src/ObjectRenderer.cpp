...
void ObjectRenderer::Free() {
    for (unsigned int i = 0; i < mShapes.size(); i++) {
        if (mShapes[i]->shape) delete mShapes[i]->shape;
        mShapes[i]->shape = NULL;
        delete mShapes[i];
    }
    for (unsigned int i = 0; i < mBBoxShapes.size(); i++) {
        if (mBBoxShapes[i]->shape) delete mBBoxShapes[i]->shape;
        mBBoxShapes[i]->shape = NULL;
    ...
    if ((tmpTree->GetName() == "Shape")) {
        ...
        if (tmpTree->GetData().length() == 0) {
            ...
            if (ptokens.size() == 1) {
                ...
                shape->shape->GenerateCapsule(atof(ptokens[0].c_str()), atoi(ptokens[1].c_str()),
                                              atoi(ptokens[2].c_str()));
            } else {
        ...
        if (tmpTree->Find("Origin")) {
            ...
            if (size == 3) {
                shape->refShape.SetOrigin().Set(array);
            } else {
        ...
        if (tmpTree->Find("Orient")) {
            ...
            if (size == 9) {
                shape->refShape.SetOrient().Set(array);
                shape->refShape.SetOrient().Normalize();
            } else if (size == 3) {
                shape->refShape.SetOrient().SRotationV(Vector3(array));
            } else {
        ...
        if (tmpTree->Find("Color")) {
            ...
            if (size == 3) {
                memcpy(shape->color, array, 3 * sizeof(REALTYPE));
                shape->color[3] = 1.0;
            } else if (size == 4) {
                memcpy(shape->color, array, 4 * sizeof(REALTYPE));
            } else {
                shape->color[0] = 1.0;
                shape->color[1] = 1.0;
                shape->color[2] = 1.0;
...
void ObjectRenderer::AddShapeFromRay(unsigned uShapeIndex, Vector3 vecRay1, Vector3 vecRay2, double dFactor,
                                     double dSphereScale) {
    ...
    if (mObject) {
        ...
        if (mShapes[uShapeIndex]->scale[0] != 1.0 || mShapes[uShapeIndex]->scale[1] != 1.0 ||
            mShapes[uShapeIndex]->scale[2] != 1.0) {
            Matrix3 matScale;
...
bool ObjectRenderer::ComputeRayIntersection(Vector3 vecRay1, Vector3 vecRay2, double &dFactor, bool bIntersections,
                                            unsigned *pShapeIndex) {
    dFactor = R_INFINITY;
    bool bFound = false;
    double dTmpFactor;
    ...
    for (unsigned i = 0; i < mShapes.size(); i++) {
        if ((mShapes[i]->strShapeName == mStrIntersectsShapeName) == bIntersections) {
            ...
            bool bFoundTmp = mShapes[i]->shape->ComputeRayIntersection(vecRay1, vecRay2,
                                                                       mObject->GetReferenceFrame().GetHMatrix(),
                                                                       dTmpFactor);
