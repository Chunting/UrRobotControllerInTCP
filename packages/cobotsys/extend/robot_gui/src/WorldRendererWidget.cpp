...
WorldRendererWidget::WorldRendererWidget(pXmlTree config, QWidget *parent)
    ...
    switch (mAntialiasingLevel) {
        case 2:
            desiredFormat.setSamples(1);
            break;
        case 3:
            desiredFormat.setSamples(2);
            break;
        case 4:
...
#else
        image = FreeImage_ConvertFromRawBits(rec_one_frame, mGLWidth, mGLHeight, 3 * mGLWidth, 24, FI_RGBA_RED_MASK,
...
void WorldRendererWidget::DrawHelp() {
    ...
    sprintf(txt, "<key>   : <action>");
    GLT::DisplayText(0, 2, txt, 1);
    sprintf(txt, "-------------------------------------------");
    GLT::DisplayText(0, 3, txt, 1);
    sprintf(txt, "h       : this help");
    GLT::DisplayText(0, 4, txt, 1);
    sprintf(txt, "SPACE   : run/pause simulation");
    GLT::DisplayText(0, 5, txt, 1);
    sprintf(txt, "s       : run a step of 0.1 sec");
    GLT::DisplayText(0, 6, txt, 1);
    sprintf(txt, "r       : reset the world");
    GLT::DisplayText(0, 7, txt, 1);
    sprintf(txt, "w       : reset the world but the robot");
    GLT::DisplayText(0, 8, txt, 1);

    sprintf(txt, "v       : toggle video recording state");
    GLT::DisplayText(0, 9, txt, 1);

    sprintf(txt, "1       : toggle stats");
    GLT::DisplayText(0, 11, txt, 1);
    sprintf(txt, "2       : toggle shadows");
    GLT::DisplayText(0, 12, txt, 1);
    sprintf(txt, "3       : toggle outlines");
    GLT::DisplayText(0, 13, txt, 1);
    sprintf(txt, "4       : toggle bounding boxes");
    GLT::DisplayText(0, 14, txt, 1);
    sprintf(txt, "5       : toggle transparency");
    GLT::DisplayText(0, 15, txt, 1);
    sprintf(txt, "6       : toggle camera look at point");
    GLT::DisplayText(0, 16, txt, 1);
    sprintf(txt, "7       : toggle reference frames");
    GLT::DisplayText(0, 17, txt, 1);
    sprintf(txt, "f       : toggle slow frame rate");
    GLT::DisplayText(0, 18, txt, 1);
    sprintf(txt, "c       : credits");
    GLT::DisplayText(0, 19, txt, 1);
