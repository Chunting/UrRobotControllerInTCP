...
int main(int argc, char **argv) {
    ...
    qDebug() << myApp.applicationDirPath();
    qDebug() << QDir::currentPath();

    // Use Ros function to get rtk_pkg_tools path.
    //std::string path = ros::package::getPath("rtk_pkg_tools"); // No need
    // Wow, so much skill.

    FileFinder::AddBasePath("/..");
    FileFinder::AddBasePath(".");
    FileFinder::AddBasePath("./data");
    FileFinder::DumpExistBasePaths();
    ...
    XmlTree argStruct("args", "", 11,
                      new XmlTree("fullscreen", "f", "needArg=\"\""),
