...
void Console::EraseChar(bool bkw) {
    ...
    if (bkw) {
        if ((m_CurrCmd.size() > 0) && (m_CursorPos > 0)) {
            ...
            m_CurrCmd = m_CurrCmd.substr(0, m_CursorPos - 1) +
                        m_CurrCmd.substr(m_CursorPos, m_CurrCmd.size() - m_CursorPos);
            MoveLeft();
        }
    } else {
        if ((m_CurrCmd.size() > 0) && (m_CursorPos < (int) m_CurrCmd.size())) {
            m_CurrCmd = m_CurrCmd.substr(0, m_CursorPos) +
                        m_CurrCmd.substr(m_CursorPos + 1, m_CurrCmd.size() - m_CursorPos);
        }
    }
}
