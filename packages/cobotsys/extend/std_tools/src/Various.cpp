...
deque<string>    FileFinder::mBasePaths;
deque<string>    FileFinder::mAdditionalPaths;
string           FileFinder::mCurrentFile = "";

bool FileFinder::Find(const char *filename) {
    string s(filename);
    return Find(s);
}
bool FileFinder::Find(string filename) {
    for (deque<string>::iterator it = mAdditionalPaths.begin(); it != mAdditionalPaths.end(); it++) {
        string cFile = *it;
        cFile.append("/");
    ...
    for (deque<string>::iterator it = mBasePaths.begin(); it != mBasePaths.end(); it++) {
        string cFile = *it;
        cFile.append("/");
        cFile.append(filename);
        if (FileExists(cFile)) {
...
const char *FileFinder::GetCStr() {
    return mCurrentFile.c_str();
}
string FileFinder::GetString() {
    return mCurrentFile;
}

const deque<string> FileFinder::GetAllPaths() {
    deque<string> res = mAdditionalPaths;
    for (deque<string>::iterator it = mBasePaths.begin(); it != mBasePaths.end(); it++) {
        res.push_back(*it);
    }
    return res;
}
void FileFinder::SetBasePaths(deque<string> &paths) {
    mBasePaths = paths;
}
void FileFinder::AddBasePath(string newPath) {
    mBasePaths.push_front(newPath);
}
void FileFinder::ClearBasePaths() {
    mBasePaths.clear();
}
void FileFinder::AddAdditionalPath(string newPath) {
    mAdditionalPaths.push_front(newPath);
}
void FileFinder::ClearAdditionalPaths() {
    mAdditionalPaths.clear();
}
string FileFinder::GetTopLevelPath() {
    for (deque<string>::iterator it = mAdditionalPaths.begin(); it != mAdditionalPaths.end(); it++) {
        return *it;
    }
    for (deque<string>::iterator it = mBasePaths.begin(); it != mBasePaths.end(); it++) {
...
void FileFinder::DumpExistBasePaths() {
    for (const auto &path : mBasePaths) {
