...
int XmlTree::LoadFromFile(string filename) {
    ...
    if (file.is_open()) {
        ...
        if (result == XTREE_ERROR) {
            fprintf(stderr, "XTree Error: while reading file: %s\n", filename.c_str());
        }
        file.close();
    } else {
            ...
            case XTREE_START_TAG: {
                ...
                if (flag == XTREE_SKIP) {
                    ...
                    if (newField->GetName() == "Instance") {
                        ...
                        if (templates != NULL) {
                            pXmlTree myTempl = templates->Find(newField->GetParamValue("type"),
                                                               newField->GetParamValue("name"));
                            if (myTempl != NULL) {
                    ...
                    fprintf(stderr, "XTree Error: Tag not closed properly: <%s> with data: %s\n", m_Name.c_str(),
                            m_Data.c_str());
                    delete newField;
                    return XTREE_ERROR;
                }
                    ...
                    if (m_Name.compare("Include") == 0) {
                        ...
                        if (GetParamValue("file").length() > 0) {
                            ...
                            if (!FileExists(newFile))
                                cout << "XTree Error: file " << newFile << " not found" << endl;
...
void XmlTree::Patch(pXmlTree patchTree) {
    ...
    for (unsigned int i = 0; i < patchTree->m_SubTrees.size(); i++) {
        ...
        if ((subTree != NULL) && (used.find((size_t) subTree) == used.end())) {
            subTree->Patch(patchTree->m_SubTrees[i]);
            used.insert((size_t) subTree);
        } else {
            pXmlTree newST = patchTree->m_SubTrees[i]->Clone();
            AddSubTree(newST);
            used.insert((size_t) newST);
        }
    }
}
...
void XmlTree::SetDataPtr(void *ptr) {
    m_DataPtr = ptr;
}
void *XmlTree::GetDataPtr() {
    return m_DataPtr;
}
