...
bool SimulatorDynamicsInterface::LoadConfig(pXmlTree config) {
    ...
    //std::string path_rtkpkgtools = ros::package::getPath("rtk_pkg_tools"); //这是一个Hack CODE,所以去掉了
    ...
    if ((tree = config->Find("World")) != NULL) {
        if (tree->GetData().length() > 0) {
            if (!mWorld->Load(tree->GetData())) {
                gLOG.AppendToEntry("Messages", "Error while opening and reading world file: %s",
                                   tree->GetData().c_str());
                bError = true;
                FreeWorld();
                return false;
        ...
        if (tlist->at(i)->GetName() == "Robot") {
            ...
            if (nameAndPatches.size() > 0) {
                ...
                if (!cRobot->Load(nameAndPatches[0], Serialize(nameAndPatches, 1))) {
                    gLOG.AppendToEntry("Messages", "Error: While operning and reading robot file <%s>",
                                       nameAndPatches[0].c_str());
                    delete cRobot;
                    cRobot = NULL;
                    bError = true;
                } else {
                            ...
                            if (FileFinder::Find(modName)) {
                                ...
                                if (dynamic_cast<RobotInterface *>(interface) != NULL) {
                                    ...
                                    if (tlist2->at(j)->Get("Name", string("")).length() > 0) {
                                        interface->SetName(cRobot->GetName() + string("_") +
                                                           tlist2->at(j)->Get("Name", string("")));
                                    } else {
                    ...
                    if (tree->Find("Orient")) {
                        ...
                        if (size == 9) {
                            ...
                            cerr
                                    << "Error: Bad robot <Orient> array size (should be 3(axis*angle) or 9(full rotation matrix))"
                                    << endl;
                        }
                    }
