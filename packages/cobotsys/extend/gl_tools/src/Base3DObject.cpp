    ...
    while (!ifile.eof()) {
        if (cline[0] == 'v') {
            ...
            if (cnt > 2) {
                ...
                polyIndexCnt += cnt;
                mNbPolygons++;
                if ((mNbPolygons >= sBASE3DOBJ_FILE_MAX_TRIANGLES) ||
                    (triPos - sTriData >= sBASE3DOBJ_FILE_MAX_TRIANGLES * 3)) {
...
void Base3DObject::CalcNormals(bool perPolyVertex, bool perVertex){
    ...
    if (bNormalsPerVertex) {
        ...
        int cumSize = 0;
        for (int i = 0; i < mNbPolygons; i++) cumSize += mPolygonSize[i];
...
void Base3DObject::CalcPlanes(){
    ...
    for (int i = 0; i < mNbPolygons; i++) {
        ...
        mPlanes(i, 3) = -(v1[0] * (v2[1] * v3[2] - v3[1] * v2[2]) + v2[0] * (v3[1] * v1[2] - v1[1] * v3[2]) +
...
bool RayTriangleIntersect(const Vector3& p1, const Vector3& p2, const Vector3& v0, const Vector3& v1, const Vector3& v2,
                          double& t){
...
bool
Base3DObject::ComputeRayIntersection(Vector3 vecRay1, Vector3 vecRay2, const Matrix4& objTransform, double& dFactor){
    ConvertToTriangles();
...
void Base3DObject::GenerateHeightField(const Matrix& heightData, REALTYPE sx, REALTYPE sy, REALTYPE sz){
    Free();
    mNbVertices =
            heightData.RowSize() * heightData.ColumnSize() + 2 * heightData.RowSize() + 2 * heightData.ColumnSize() - 4;
    mNbPolygons = +1 + 2 * (heightData.ColumnSize() - 1) + 2 * (heightData.RowSize() - 1) +
                  (heightData.RowSize() - 1) * (heightData.ColumnSize() - 1);
