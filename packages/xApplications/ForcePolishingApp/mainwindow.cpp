...
MainWindow::MainWindow() : m_ticks(0.0) {
    ...
    m_polishController.reset(new PolishController());
    ...
    connect(m_polishController.get(), SIGNAL(jointUpdated(StdVector)), this, SLOT(onJointUpdated(StdVector)));
    connect(m_polishController.get(), SIGNAL(poseUpdated(StdVector)), this, SLOT(onPoseUpdated(StdVector)));
    connect(m_polishController.get(), SIGNAL(forceUpdated(MyWrench)), this, SLOT(onForceUpdated(MyWrench)));
    ...
    m_polishController->setup("CONFIG/UrRobotConfig/ur3_180_config.json");
    initializePlot();
    m_joints.clear();
    m_pose.clear();
    for (int i = 0; i < 6; i++) {
...
void MainWindow::onSettingCalib(){
    m_polishController->setControllerStatus(PolishController::CALIB);
