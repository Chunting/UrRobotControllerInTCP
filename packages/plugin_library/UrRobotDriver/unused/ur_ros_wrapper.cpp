...
#include "../src/ur_driver.h"
#include "ur_hardware_interface.h"
#include "../src/do_output.h"
#include <string.h>
#include <vector>
#include <mutex>
    ...
    void rosControlLoop(){
        ...
        while (ros::ok()) {
            ...
            elapsed_time = ros::Duration(
                    current_time.tv_sec - last_time.tv_sec + (current_time.tv_nsec - last_time.tv_nsec) / BILLION);
...
int main(int argc, char** argv){
    ...
    if ((ros::param::get("~reverse_port", reverse_port))) {
        if ((reverse_port <= 0) or (reverse_port >= 65535)) {
            print_warning(
                    "Reverse port value is not valid (Use number between 1 and 65534. Using default value of 50001");
            reverse_port = 50001;
        }
    } else
        reverse_port = 50001;
