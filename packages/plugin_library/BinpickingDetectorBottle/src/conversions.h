...
namespace cv_ext {
    ...
    inline void exp2RotMat(const cv::Mat &r_vec, cv::Mat &r_mat) {
        if (r_vec.rows < 3)
            throw std::invalid_argument("Input matrix has wrong size");
    ...
    template<typename _T>
    inline void exp2RotMat(const cv::Vec<_T, 3> &r_vec, cv::Mat &r_mat) {
        cv::Mat_<_T> tmp_r_vec(r_vec);
        cv::Rodrigues(tmp_r_vec, r_mat);
    }

    template<typename _T>
    inline void exp2RotMat(const cv::Mat &r_vec,
    ...
    template<typename _T>
    inline void exp2RotMat(const cv::Vec<_T, 3> &r_vec,
                           Eigen::Matrix<_T, 3, 3> &r_mat) {
        cv::Mat_<_T> tmp_r_vec(r_vec), tmp_r_mat;
        cv::Rodrigues(tmp_r_vec, tmp_r_mat);
        openCv2Eigen<_T, 3, 3>(tmp_r_mat, r_mat);
    ...
    template<typename _T>
    inline void rotMat2Exp(const cv::Mat &r_mat, cv::Mat &r_vec) {
    ...
    template<typename _T>
    inline void rotMat2Exp(const cv::Mat &r_mat, cv::Vec<_T, 3> &r_vec) {
        if (r_mat.rows < 3 || r_mat.cols < 3)
            throw std::invalid_argument("Input matrix has wrong size");
    ...
    template<typename _T>
    inline void rotMat2Exp(const Eigen::Matrix<_T, 3, 3> &r_mat,
                           cv::Mat &r_vec) {
    ...
    template<typename _T>
    inline void exp2TransfMat(const cv::Mat &r_vec,
                              const cv::Mat &t_vec,
                              cv::Mat &g_mat) {
    ...
    template<typename _T>
    inline void exp2TransfMat(const cv::Vec<_T, 3> &r_vec,
    ...
    template<typename _T>
    inline void transfMat2Exp(const cv::Mat &g_mat,
